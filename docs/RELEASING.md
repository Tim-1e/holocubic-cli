...
`cubic-rs`. It uses crates.io Trusted Publishing. Confirm the name and version
in `implementations/rust/Cargo.toml`, then run:
...
Configure its crates.io Trusted Publisher with these exact values:
...
After the checks pass, commit and push the release change, then create a tag
that exactly matches `rust-v<Cargo.toml version>`. For example, version `0.2.0`
uses:
...
```sh
cargo install holocubic-cli-rust --version 0.2.0 --locked --root ./target/registry-smoke
